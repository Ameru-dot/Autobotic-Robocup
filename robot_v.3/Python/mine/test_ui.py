...
class DummyState:
    def __init__(self):
        ...
        self.img2 = np.zeros((480, 640, 3), dtype=np.uint8)
        self.light_on = False
    ...
    def tick(self):
        while self.running:
            ...
            self.objective = "follow_line" if int(self.t) % 2 == 0 else "zone_search"
...
def build_ui(state: DummyState):
    ...
    ttk.Button(btn_frame, text="Mode: Zone", command=lambda: state.__setattr__("objective", "zone_search")).pack(side="left", padx=4)
    ...
    ttk.Button(calib_frame, text="Calib Line Green", command=lambda: state.__setattr__("status", "Calib line green")).pack(side="left", padx=2)
    ttk.Button(calib_frame, text="Calib Line Black", command=lambda: state.__setattr__("status", "Calib line black")).pack(side="left", padx=2)
    ttk.Button(calib_frame, text="Calib Line Red", command=lambda: state.__setattr__("status", "Calib line red")).pack(side="left", padx=2)
    ttk.Button(calib_frame, text="Calib Zone Green", command=lambda: state.__setattr__("status", "Calib zone green")).pack(side="left", padx=2)
    ttk.Button(calib_frame, text="Calib Zone Red", command=lambda: state.__setattr__("status", "Calib zone red")).pack(side="left", padx=2)
