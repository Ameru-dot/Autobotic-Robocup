...
# ??PWM(??????????) Control PWM (for motors without encoder)
...
def parse_data(data):
    ...
    if data.startswith("$MAll:"):
        values_str = data[6:-1]  # ?? "$MAll:" ? "#" Remove "$MAll:" and "#"
...
def set_motor_parameter():
    ...
    if MOTOR_TYPE == 1:
        ...
        time.sleep(0.1)
        set_pluse_phase(30)  # ?????,???????   Configure the reduction ratio and check the motor manual for the result.
        time.sleep(0.1)
        set_pluse_line(11)  # ?????,???????    Configure the magnetic ring wire and check the motor manual to get the result
        time.sleep(0.1)
        set_wheel_dis(67.00)  # ??????,????  Configure the wheel diameter and measure it
        time.sleep(0.1)
